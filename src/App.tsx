...
function App() {
  ...
  const [logginUser, setLoggin] = useState<InewUser>({
    username: "",
    password: "",
    subscribe: false,
  });
  ...
  function sendToApi() {
    axios
      .post<InewUser[]>("http://localhost:3000/users/add", newUser)
      .then((res) => {
  ...
  const handleLoggin = () => {
    axios
      .post("http://localhost:3000/users/loggin", {
        username: username,
        password: password,
      })
      .then((response) => {
  ...
  return (
    <div className="App">
      ...
      <form>
        ...
        />
        <br />
        <button onClick={handleLoggin}>skicka</button>
        <br />
